...
- `WaveSystem`: 적은 그리드 우측 열(본진 반대편) 바깥 가상 칸에서 슬라이드해 들어와 매 틱(1.3초) 확률적으로 전진 0.4/배회 0.4/대기 0.2로 움직이며, 아군이 있는 칸에도 자유롭게 진입한다. 웨이브 3기는 한꺼번에가 아니라 랜덤 행 순서로 340~860ms 간격 순차 등장(`spawnWaveStaggered`, 대기 타임아웃은 `halt()`가 취소). 웨이브는 30초마다 강제로 밀려오고, 그 전에 필드의 적을 모두 처치하면 즉시 다음 웨이브가 밀려온다. 5웨이브마다 체크포인트 — 소강이 먼저 걸리고 해당 웨이브는 `resumeFromCheckpoint()`에서 도착한다(정비 중 선스폰 금지). 3번째 체크포인트마다 유물 3택 화면. 클래시 해소는 이동 틱 직후가 아니라 900ms 뒤(토큰 슬라이드 0.85s 완료 후)에 실행되고, `BoardRenderer`는 880ms 내에 움직인 토큰의 런지 연출을 스킵해 이동/전투 애니메이션이 충돌하지 않는다.
- 트리플 머지: 같은 크리처 기본(tier 미지정) 카드 3장이 손패에 모이면 자동 합성 — `HandSystem.findTriple/mergeTriple` + `CardHand.playMergeFx`(바깥 2장 탄성 수렴 0.4s → 젤리 스쿼시+반짝 0.54s → 모델 교체 pop). 결과는 `tier: 2` 카드 1장으로 보라 링/글로우로만 구분되는 **플레이스홀더** — 진화체 20종 로스터/능력치 연결은 미구현.
...
- `IntroOverlay` 시작 버튼은 플레이버 문구("심연이 그대를 기다린다") + 대형 보더리스 버튼 구성으로, 체크포인트 `proceed-pulse`와 다른 느린 `intro-breathe`(2.6s) 발광을 쓴다.

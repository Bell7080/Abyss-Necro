...
- `SkillHive`(웨이브 HUD 아래 화면 중앙): 가로 헥스 4종(공격/급조/기상/포획, 간격 26px) + 가로 사령 게이지. 헥스 줄 뒤에는 별자리 점무늬 + 은은한 보라 후광 배경이 깔린다. 헥스에 마우스를 올리면 하이브 우측에 이름·대사·게이지 소모량을 보여주는 안내 패널이 뜬다(`hive-tooltip`). 게이지 fill은 `AbilitySystem.getSmoothGauge()`를 rAF로 매 프레임 읽고, 헥스 disabled/armed 상태는 `render()`/`setArmed()`. 급조/포획 arm 시 앰버 발광 + 화면 슬로우, 기본공격 헥스는 항상 켜진 기본 상태(로딩 링은 헥스 테두리에 고정된 채 그 위를 도는 하이라이트로 표현 — 링 자체가 통째로 도는 게 아니라 `conic-gradient`의 시작각(`--hex-spin-angle`, `@property` 등록)만 애니메이션). 기본공격/급조가 보드·유해 직접 클릭으로 발동되면 해당 헥스가 확대·발광하는 `flashCast()` 피드백을 준다.

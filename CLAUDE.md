...
- 최초 스캐폴드 단계: 실제 게임 로직 미구현, Vite+Electron 파이프라인과 플레이스홀더 그리드 보드(`src/ui/BoardRenderer.ts`)만 존재.
- 카드/합성/사령/도감 시스템, 40종 적·40종 카드·20종 진화체·5종 보스 데이터 테이블은 모두 미구현 — `Abyss_Necro_Game_Concept.md`의 코어 루프 정의를 기준으로 다음 세션에서 구현한다.

...
- 조준 모드: 손패 카드 선택 중 화면 전체 어둠(aim-dim, 보드만 z-index로 부상) + 배치 칸 beckon 펄스 + 0.3배속 + **우측** 카드 인스펙터(`CardInspector`, Unmelting 거점 인스펙터 해부도 우측 미러 — 제목/태그/설명). 유물 독은 우하단 고정이되 z-index 96으로 인스펙터 위에 겹쳐 항상 보인다.

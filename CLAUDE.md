...
- `src/assets` — sprites(webp)/audio(mp3)/fonts(woff2). `sprites/`는 `enemies/`(적, before)·`allies/`(사령된 아군, after)·`backgrounds/`(배경) 하위 폴더로 분리. **크리처 아트는 id 기반 자동 연결**: `CreatureDefinitions.ts`가 `import.meta.glob`로 `enemies/<id>.webp`·`allies/<id>.webp`를 이름으로 매핑하므로, 파일명을 크리처 id(`jellyfish`, `shark`…)로 넣기만 하면 점등되고 없으면 빌드가 안 깨지고 폴백 워터마크로 뜬다. **합성 등급(2성/3성) 전용 아트도 같은 폴더에 `<id>-2.webp`/`<id>-3.webp`로 추가**하면 `getAllyArt(id, tier)`가 우선 집어 쓰고(없으면 한 단계 아래 등급 아트로 자동 폴백, 그것도 없으면 1성 원화), 보드 배치 아군·손패 카드·도감이 모두 이 함수로 등급별 아트를 그린다. 현재 해파리/바다토끼/꽃게 3종(`jellyfish`/`sea-rabbit`/`crab`, before+after 1성)만 실제 원화가 있고 나머지 16종은 아트 대기 중. 배경(`bg_001`)·플레이어(`player_001`)도 있다.
...
- 메인 화면 골격 구현됨: 3D로 기울인 중앙 보드판(좌측 본진 1칸 + **5열×3행** 적 그리드=3레인(가로 5칸 깊이), 칸 200px, `--board-tilt` CSS 변수), 하단 손패 부채꼴, 우측 하단 유물 인벤토리, 우측 상단 도감 런처, **웨이브 HUD 바로 아래 화면 중앙에 가로 스킬 헥스 4종 + 가로 사령 게이지**(`SkillHive` — 화면 중앙이 시인성이 좋아 플레이어 카드 옆에서 이 위치로 이동, 판 기울기와 무관한 평면 HUD). 그리드 크기는 `WaveSystem`/`DefenderSystem`의 `ROWS/COLS`와 `BoardRenderer`의 `GRID_ROWS/GRID_COLS/CELL_SIZE`가 공유하고, `constellationSvg`는 이 값으로 임의 행×열을 그린다. 좌측 본진 칸의 플레이어와 아군은 칸 안쪽이 아니라 방의 왼쪽 끝에 걸치듯 멀리 배치(`--board-figure-shift-x`/`BOSS_ALLY_OFFSET_X`)해 본진 안에 온전히 들어와 있는 느낌을 낸다. 보드판 전체는 화면 정중앙보다 살짝 좌측으로 당겨져(`.board-mount`) 본진이 화면 좌측 벽에 붙는 느낌을 내고, 본진 칸(`.board-player-cell`)도 그리드 칸과 같은 인셋 그림자 + 약간의 아래 이동으로 그리드 바닥에 실제로 앉아있는 느낌을 준다(전엔 살짝 떠 보였음). 화면 전체 우측 끝은 `AbyssAmbience`의 `abyss-ambience-rightfog`(전체 뷰포트 높이, 위아래 잘림 없음)로 짙은 어둠에 잠겨 적이 심해에서 올라오는 느낌을 준다. 레인 사이를 잇던 화살표는 각 레인 입구에서 본진으로 별자리 선이 한 점(다이아 장식)으로 모이는 연출로 대체됐다(`convergeSvg`). 웨이브 첫 스폰은 보이는 입구 칸보다 3칸 더 밖(안개 저 너머)에서 시작해 2.6초에 걸쳐 천천히 기어오는 느낌으로 등장(`SPAWN_APPROACH_COLUMNS`/`.is-spawning-approach`)해 플레이어가 미리 보고 대비할 수 있게 한다. 적 스탠디는 아군보다 칸 안에서 살짝 더 위쪽에 서도록(`.board-figure:has(.entity-card--enemy)`) 배치해 "칸 위에 얹힌" 느낌 대신 "서 있는" 느낌을 낸다.
- `IntroOverlay`: 게임 진입 시 보드가 아닌 별도 타이틀 스크린(배경 아트 `bg_002`)을 먼저 보여준다. 완전한 검은 화면(`.intro-black-veil`)이 2.8초에 걸쳐 걷히며 타이틀 아트 + 캐스틱 물결(`.abyss-caustic` 재사용) + 강한 비네틱 + 큼직하고 선명하게 반짝이는 반딧불이(`.intro-firefly`)가 드러나고, 다 드러난 뒤(`onRevealed` 콜백)에야 OST 재생이 시작되며(6초 페이드인) 하단에 "화면을 눌러 심해의 친구들을 사귀러 떠나기!"가 점멸한다. 화면 아무 곳이나 클릭하면 장면 전환처럼 화면이 다시 검게 컷되며(`onDismiss` 콜백에서 `AudioManager.duckTitleOst()`가 OST를 "투-후-후-훙" 스타터 컷으로 끊는다) 그 암전 뒤에서 다시 페이드인되어 아직 정지된 채인 보드가 드러난다 — **"시작하기" 버튼은 더 이상 없다.** 이 페이드가 끝나면 오버레이가 스스로 사라지고 `onBoardRevealed` 콜백이 `Game.startIntroDialogue()`를 호출해 넥슈의 오프닝 대화(`IntroDialogue`)로 자동 인계된다.
- `IntroDialogue`(넥슈 오프닝 컷신, `src/ui/IntroDialogue.ts`): 보드 위에 어두운 반투명 딤(`.intro-dlg-dim`)이 페이드인되고 그 위로 하단을 거의 가득 채우는 청색 라인 대화창이 뜬다. 대화창 위쪽 중앙에서 큼직한(원래의 2배) 표정 일러스트(`player_002`=기본/`player_003`=웃음/`player_004`=울음)가 페이드인되며, 뒤에 같은 그림을 살짝 좌측으로 빼고 반투명하게 둔 잔상(`.intro-dlg-portrait-shadow`)이 두겹처럼 보이게 한다. 대화창은 중앙 발광 이름 + 발광 실선 구분선 + 중앙정렬 대사로 구성되고, 대사는 한 글자씩 튀어 오르듯 팝인하는 바운스 타자 효과로 출력되며(`TYPE_MS`, 글자별 `<span>`+`intro-dlg-char-pop` 키프레임), 다 출력된 뒤엔 대사 전체가 은은하게 부유하는 느낌(`is-line-settled`)을 준다. 화면 클릭 시 즉시 완성되고, 완성 후 0.75초(`ADVANCE_LOCK_MS`)는 클릭이 무시되다가 그 뒤 클릭 또는 6초(`ADVANCE_TIMEOUT_MS`) 경과 시 다음 대사로 넘어간다. 표정 전환은 잔상과 함께 빠르게 스르륵 바뀐 뒤 이전 그림이 지워지는 크로스페이드(`is-swap-out`/`is-swap-in`)로 처리된다. 스크립트 중간에 넥슈 프로필/대화 정렬이 좌측으로 밀리며(`is-shifted`) 화면 중앙에 해파리/피라냐/문어 적 카드가 흩뿌려지고(`entityCardHtml` 재사용, 그림자 포함), 보글보글 버블 블라스트(`.intro-card-bubble`)와 함께 사령된 아군 카드로, 다시 2성으로 업그레이드된 뒤 페이드아웃되고, 이어서 해파리/소라게/꽃게 3성 카드가 의미심장하게 등장했다 사라진다. 마지막 대사 이후 딤이 걷혀 보드가 드러나고 대화창/일러스트가 사라진 뒤 화면 중앙에 5·4·3·2·1 카운트다운이 뜨고, "1"이 웨이브 HUD의 시간 표시(`WaveHud.getTimeRect()`)로 날아가 부딪히는 순간 `onComplete` 콜백이 `Game.startRun()`을 호출해 실제로 웨이브/게이지가 시작된다. 표시/연출 전담이며 실제 상태 변경(`WaveSystem.start()` 등)은 전부 `Game.startRun()`에 남아 있다.
- `WaveSystem`(레인 러시): 각 그리드 행이 레인(3레인, 각 레인 가로 5칸 깊이). 웨이브는 여러 레인에 분산 스폰(랜덤 시작 레인부터 라운드로빈)되어 매 틱(1.3초) **자기 레인을 따라 무조건 왼쪽 전진**(배회 없음) — 밀물처럼 압박한다. 아군이 있는 칸에선 전진을 멈추고 교전(그게 레인 막기). **웨이브 구성은 절차적 완만 램프**(`waveRoster`/`focusLevel`/`waveCount`, `REGULAR_LADDER` 14종): **1~8웨이브는 순수 해파리만**(`PURE_OPENING_WAVES`=`FIRST_TIER_WAVES`, 믹스 없음) 나와 첫 체크포인트 전에 무덤·손패 양쪽에 해파리가 넉넉히 쌓여 첫 트리플 합성을 해볼 수 있고, 9웨이브부터 바다토끼가 섞이기 시작한다. 그 뒤로 포커스 레벨은 **`TIER_WAVES`(4)웨이브마다 한 단계씩** 올라(클라운피시=13웨이브, 새우=17웨이브...) 약→강 서서히 깊어진다. 새 티어는 그냥 켜지듯 등장하지 않고 **자기 자신의 4웨이브 구간 안에서 이전 티어와 서서히 크로스페이드**된다(`tierProgress`+`pickRegularLevel`: 구간 시작엔 새 티어 15%/이전 85%, 끝엔 새 티어 85%/이전 15% — 조금→반반→많이 순으로 서서히 역전), 거기에 포커스가 3티어 이상이면 그보다도 더 오래된 하위 티어 전부에 고르게 분산되는 15% 몫도 얹혀 후반에도 로스터가 다채롭다. **마릿수는 웨이브 번호와 1:1**(`waveCount`, 웨이브 1=1마리·2=2마리…20마리 상한)로 늘어 후반으로 갈수록 확실히 붐빈다. **적 스탯은 크리처 레벨**(`Tiers.enemyStatsForLevel`, `LEVEL_HP/ATK`)에서 나온다. 30초마다 강제 푸시, 필드 클리어 시 5초 숨고르기 후 푸시. **10웨이브마다 체크포인트**(`SHOP_EVERY`) — 소강 먼저, 해당 웨이브는 `resumeFromCheckpoint()`에서 도착. 이동 칸당 슬라이드는 1.15초(거의 매 틱을 채워 뚝뚝 끊기지 않는 연속된 기어오는 느낌)이고 클래시 해소는 그 뒤 1.2초에 일어난다. 조준/스킬 arm 0.3배속은 `TickManager.setRate`+`WaveSystem.setTimeScale`.
...
- **도감(`CodexOverlay`)**: 우측 상단 책 런처(Unmelting `compendium-btn` hover 이식 — 아이콘 발광·라벨 페이드). 클릭 시 반투명 짙은 청색 창이 떠오르고, **크리처 1종당 한 슬라이드**로 큰 풀 일러스트 **적(before, 좌) → 사령 → 사령된 아군(after, 우)** + 공격/체력 스탯 칩 + 설명을 보여준다(냉기 청색 링 → 온기 보라/오렌지 링). **사령(after) 카드 상단에 ★/★★/★★★ 등급 탭**이 있어 누르면 그 등급의 아트(`getAllyArt`)와 스탯(`allyStatsForLevel(level, tier)`)으로 즉시 갈아끼운다(패시브 설명은 등급과 무관해 그대로). 창을 열 때마다 전 크리처 1성으로 초기화. 하단 위/아래 셰브런 + `n / N` 카운터, `scroll-snap`으로 한 몹씩 넘긴다("내리면 다음 몹"). 표시 전용, 상태 변경 없음. 적 설명은 `CreatureDefinition.enemyDesc`.
- 드롭: 처치 시 **33% 온전한 손패 카드(그 크리처 사령) / 67% 유해**(`CorpseSystem`) + 별빛 1개 확정 + 사령 게이지 +2. 1웨이브 처치는 카드 확정(튜토리얼). 유해는 급조 대상이거나 방치 시 3.8초 뒤 75%로 묘지 조각. **아이템 카드는 이 카드/유해 굴림과 별개로 처치마다 12%의 추가 확률**(`ITEM_DROP_CHANCE`, `Game.handleEncounter`)로 한 장 더 드롭된다(에픽은 여전히 **상점에서만**). 아이템(`ItemCardDefinitions`): 심연 파동(전체 1피해)/치유 물결(칸 아군 3회복). 에픽(`EpicCardDefinitions`) 5종: 함정별/증가별(아군 공격 +1)/심연의 심장(최대체력 +2)/벼려진 저주(기본공격 +1)/과부하 문양(**최대 사령 게이지 +2**). 아이템/에픽은 합성 제외, 즉시 소모.
- 심연 상점(`ShopOverlay`): 일반 체크포인트마다 좌우 양쪽에서 은은한 어두운 물 안개(`.shop-fog`)가 밀려와 필드를 가득 채우며 어둡게 만들고, 그 위로 중앙에서 반짝이며 패널이 펼쳐진다 — 랜덤 3슬롯, 별빛으로 구매(`CoinSystem.spend`) 후 손패로 버블 전달. **밀어낸 깊이(체크포인트의 웨이브)에 비례해 오퍼가 좋아짐**: 사령 풀은 `levelCap`(깊이↑ = 레벨 상한↑, 상어 제외)에서 레벨 가중 추첨, 강한 사령은 가격 소폭↑(✦2~4), 에픽 확률도 깊이 따라 상승. 3번째 체크포인트(=30웨이브, 보스 직후)는 유물 3택(`RewardOverlay`). 유물탭은 항상 보이는 우측 하단 고정 독(`.relic-dock`). **체크포인트 소강 중엔 스킬/손패 사용이 전부 잠기고**(`WaveSystem.isPaused()`로 `Game.handleSkill`/손패 선택 콜백을 가드) 합성과 상점 구매만 가능하다.
...
- 보드 비주얼: 칸은 "#" 별자리(별빛 선 + 위상차 트윙클 별, `constellationSvg`)와 은은한 그림자 웅덩이. 카드 진영 발광 — 적 적색/아군 청색/플레이어 보라(`--card-ring`/`--card-glow`), 적 카드에도 이름 표기, 피격 시 붉은 점멸(`.is-hit`), 하단 타원 접지 그림자. 유해는 연보라 어두운 마커(`board-corpse`, 은은해진 후광), 생성되는 순간엔 작은 보글보글 거품이 튀어 오르며 탄력있게 팝인(`is-spawning`)한다. 급조 언데드는 연보라 일렁 그림자(`board-figure.is-raised`). 기본공격 투사체는 직선 버블 화살 + 착탄 폭죽(`BubbleBolt`). 플레이어 카드 200px·이름 타이틀급. 적이 그리드 진입 시(스폰)는 팬텀 칸에서 슬라이드해 들어오고(`spawnFromX`), 그리드에서 플레이어 방으로 넘어갈 때는 grid↔room이 서로 다른 DOM 컨테이너/토큰 맵을 쓰는 탓에 새 엘리먼트가 생성되던 문제를 grid 쪽 옛 토큰의 화면 좌표(`getBoundingClientRect`)를 방 프루닝 전에 캡처해 room 로컬 좌표로 변환한 지점에서 시작하도록 고쳐(`BoardRenderer.syncCells`의 `crossRects`→`syncBossRole`→`upsertToken`), 실제 이동의 연속으로 슬라이드해 들어온다(텔레포트 없음).
- `PlayerSystem`(hp 18, 회복 없음)/`DefeatOverlay`: 플레이어 방에 도달한 적은 아군 디펜더가 있으면 그와 싸우고, 없으면(또는 아군이 있어도 **15% 확률로 슬쩍 지나쳐**, `PLAYER_BYPASS_CHANCE`) 매 틱 선두 1기가 넥슈를 직접 공격한다(`WaveSystem.onPlayerHit`, 플레이어 카드 체력바 실시간 반영). hp 0이 되면 `TickManager.stop()`+`WaveSystem.halt()`로 월드가 정지하고 패배 오버레이("패배 / 심해는 무서워! / 다음에 다시 올래. / 다시하기", 리로드)가 반투명 블러로 덮인다.
- `SkillHive`(웨이브 HUD 아래 화면 중앙): 가로 헥스 4종(공격/급조/기상/포획, 간격 26px) + 가로 사령 게이지, 테두리·배경 장식 없이 헥스 자체 그림자만으로 깔끔하게 표현. 헥스에 마우스를 올리면 마우스 위 말풍선이 아니라 보드/손패 hover와 같은 **우측 `CardInspector`**에 이름·게이지 비용·대사·설명이 뜬다(`Game.handleSkillHover`, `SKILL_FACES.desc`). 게이지 fill은 `AbilitySystem.getSmoothGauge()`를 rAF로 매 프레임 읽고, 헥스 disabled/armed 상태는 `render()`/`setArmed()`. 기본공격 헥스의 로딩 링은 헥스 테두리에 고정된 채 그 위를 도는 얇은 하이라이트로 표현(`conic-gradient` 시작각 `--hex-spin-angle` 애니메이션). 헥스는 `clip-path`로 육각형으로 잘리므로 바깥으로 번지는 발광은 전부 `box-shadow`가 아닌 `filter: drop-shadow()`로 그려 클리핑 경계에서 흐림이 뚝 끊기지 않게 한다(인셋 링만 `box-shadow` 유지). 급조/포획 arm 시 앰버 발광 + 화면 슬로우. 기본공격/급조가 보드·유해 직접 클릭으로 발동되면 해당 헥스가 확대·발광하는 `flashCast()` 피드백을 준다.
- `AbilitySystem`(사령 게이지): 통합 게이지(최대10, **런 시작 시 0에서 출발**, 처치당 +2 충전 + 2초당 +1 트리클 바닥). 능력 4종 `ABILITY_COST` — 기본공격 1(단일 칸 클릭, `BubbleBolt` 발사→착탄 시 `WaveSystem.applyDamage`)/급조부활 2(arm→칸, 유해 전부 기립)/전체부활 6(즉발, 필드 유해 전부)/포획 10(arm→저격, HP≤25%(보스≤10%) 즉사+확정 카드). 과부하 문양 에픽이 `increaseMaxGauge`로 최대치 상향. 전체부활/포획처럼 비용이 크고 드문 스킬은 사용 가능해지면 단순히 불이 켜지는 정도가 아니라 맥동하며 확대되는 호박색 잔상 링이 계속 도는 강한 강조(`SkillHive`의 `is-primed`)로 눈에 띈다. 클릭 모드 중재는 `Game`: 손패 선택=배치 / 스킬 arm=대상선택 / 그 외=기본공격. 임시 소환수 실험(`DefenderSystem.summon`/`stepSummons`)은 어느 능력에도 연결 안 됨(코드만 보존).
...
- 모든 보드 개체(적/아군/플레이어)는 `EntityCard`로 렌더링 — 풀 일러스트 카드 프레임(아군 128px·적 115px·플레이어 200px 폭, 적이 아군보다 한 단계 더 작음) + 하단 심해 색 체력바(숫자 없음). `CreatureDefinitions.ts`에 등록된 크리처(현재 해파리/바다토끼/꽃게 3종 + 플레이어)는 실제 원화를 `object-fit: cover`로 표시한다. `EnemyToken`/`AllyToken`/`HandCard`가 `creatureId`를 스폰→처치→카드→배치 전 구간에 들고 다녀 같은 크리처의 before(적)/after(사령된 아군) 원화가 일관되게 이어지며, 손패 카드 자체는 배치 시 실제로 얻을 사령 후(after) 모습을 미리 보여준다. 손패 카드를 칸에 배치하면 카드가 그 칸으로 날아가 팡 터지며(`BubbleBurst` travelDrop→clashBurst) 배치된다. 한 칸에 여럿(최대 3기) 겹치면 **별(등급) 높은순·그다음 공격력 높은순으로 정렬해 가장 강한 개체가 맨 앞(하단)에 서도록** 안정적으로 배치한다(`BoardRenderer.syncGridRole`/`syncBossRole`). 유해(시체)는 항상 그 칸의 다른 토큰보다 앞 레이어(`z-index`)에 그려져 가려지지 않는다. 손패도 같은 규칙(별 높은순, 동률이면 크리처별로 묶어서)으로 정렬되어 표시된다(`CardHand.sortForDisplay`).

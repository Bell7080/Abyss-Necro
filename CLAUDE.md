...
- **보스(클라이맥스)**: `BOSS_WAVE`(현재 6) 시작 시 중앙 레인에 **엘리트 해파리** 보스 한 마리를 함께 스폰(`WaveSystem.spawnBoss`, `EnemyToken.isBoss`/`label`/`attack`, 체60·공3·해파리 아트). 보드에선 `board-token.is-boss`로 카드가 더 크고 보라 오라가 숨쉰다. 포획 임계는 `EnemyToken.isBoss` 기준(보스 ≤10% / 일반 ≤25%, `WaveSystem.captureThreshold`)이며 `isCapturable`/`captureFrontEnemy`는 셀이 아닌 적으로 판정한다. 처치·포획 모두 해파리 사령 카드를 확정 지급. 아직 5종 보스 로스터·전용 아트·승리 조건은 미구현(단일 엘리트만).
- **도감(`CodexOverlay`)**: 우측 상단 책 런처(Unmelting `compendium-btn` hover 이식 — 아이콘 발광·라벨 페이드). 클릭 시 반투명 짙은 청색 창이 떠오르며 `CREATURES` 전종을 **적(before) 일러스트+설명 → 사령 → 사령된 아군(after) 일러스트+패시브** 한 행으로 비교(냉기 청색 링 → 온기 보라/오렌지 링). 표시 전용, 상태 변경 없음. 적 설명은 `CreatureDefinition.enemyDesc`.

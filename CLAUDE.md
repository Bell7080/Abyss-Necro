...
- 보드 비주얼: 칸은 "#" 별자리(별빛 선 + 위상차 트윙클 별, `constellationSvg`)와 은은한 그림자 웅덩이. 카드 진영 발광 — 적 적색/아군 청색/플레이어 보라(`--card-ring`/`--card-glow`), 적 카드에도 이름 표기, 피격 시 붉은 점멸(`.is-hit`), 하단 타원 접지 그림자. 유해는 연보라 어두운 마커(`board-corpse`), 급조 언데드는 연보라 일렁 그림자(`board-figure.is-raised`). 기본공격 투사체는 직선 버블 화살 + 착탄 폭죽(`BubbleBolt`). 플레이어 카드 200px·이름 타이틀급. 적이 그리드 진입 시(스폰)와 플레이어 방으로 넘어갈 때 둘 다 팬텀 칸에서 슬라이드해 들어온다(`spawnFromX`, 텔레포트 없음).

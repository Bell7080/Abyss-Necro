...
- `WaveSystem`(레인 러시): 각 그리드 행이 레인(3레인, 각 레인 가로 4칸 깊이). 웨이브는 여러 레인에 분산 스폰(랜덤 시작 레인부터 라운드로빈)되어 매 틱(1.3초) **자기 레인을 따라 무조건 왼쪽 전진**(배회 없음) — 밀물처럼 압박한다. 아군이 있는 칸에선 전진을 멈추고 교전(그게 레인 막기). **웨이브 구성은 절차적 완만 램프**(`waveRoster`/`focusLevel`/`waveCount`, `REGULAR_LADDER` 14종): **1~7웨이브는 순수 해파리만**(`PURE_OPENING_WAVES`, 믹스 없음) 나와 첫 체크포인트 전에 무덤·손패 양쪽에 해파리가 넉넉히 쌓여 첫 트리플 합성을 해볼 수 있고, 8웨이브부터 바다토끼가 섞이기 시작한다. 그 뒤로 포커스 레벨은 **`TIER_WAVES`(7)웨이브마다 한 단계씩** 올라(클라운피시=15웨이브, 그다음도 7웨이브 간격으로 반복) 약→강 서서히 깊어지고, 같은 저레벨이 충분히 지속돼 3장 모아 합성·필드 축적하는 재미가 이어진다. 각 웨이브 로스터는 포커스 크리처 55% + 다음 티어 살짝 미리보기 8% + **나머지는 그때까지 풀린 모든 하위 티어에 고르게 분산**(`pickRegularLevel`, 최근 1~2종만이 아니라 해파리부터 전부 다시 섞여 나올 수 있음)이라 후반에도 로스터가 다채롭다. **마릿수는 웨이브 번호와 1:1**(`waveCount`, 웨이브 1=1마리·2=2마리…20마리 상한)로 늘어 후반으로 갈수록 확실히 붐빈다. **적 스탯은 크리처 레벨**(`Tiers.enemyStatsForLevel`, `LEVEL_HP/ATK`)에서 나온다. 30초마다 강제 푸시, 필드 클리어 시 3초 숨고르기 후 푸시. **10웨이브마다 체크포인트**(`SHOP_EVERY`) — 소강 먼저, 해당 웨이브는 `resumeFromCheckpoint()`에서 도착. 클래시 해소는 이동 900ms 뒤, `BoardRenderer`는 880ms 내 런지 스킵. 조준/스킬 arm 0.3배속은 `TickManager.setRate`+`WaveSystem.setTimeScale`.

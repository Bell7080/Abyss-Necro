...
- `WaveSystem`(레인 러시): 각 그리드 행이 레인(3레인, 각 레인 가로 4칸 깊이). 웨이브는 여러 레인에 분산 스폰(랜덤 시작 레인부터 라운드로빈)되어 매 틱(1.3초) **자기 레인을 따라 무조건 왼쪽 전진**(배회 없음) — 밀물처럼 압박한다. 아군이 있는 칸에선 전진을 멈추고 교전(그게 레인 막기). **웨이브 구성은 큐레이션됨**(`WAVE_SPAWNS` id→개수 스케줄): 크리처를 **레벨 오름차순으로 순차 도입**(웨이브1 해파리 1마리 → 웨이브2 해파리+바다토끼 1 → …)해 매 웨이브가 뚜렷한 파워 스텝이 되고, 비슷한 스팩 몹이 섞이지 않는다. **적 스탯은 크리처 레벨**(`Tiers.enemyStatsForLevel`)에서 나오고 웨이브 무관 — 난이도 곡선은 높은 레벨 크리처가 등장하며 가파르게 격등한다(레벨 커브 `LEVEL_HP/ATK`는 초반 완만→후반 급증). 적 공격도 레벨값으로 개별화돼 클래시·플레이어 피격에 반영. 30초마다 강제 푸시, 필드 클리어 시 3초 숨고르기 후 푸시. **3웨이브마다 체크포인트** — 소강 먼저, 해당 웨이브는 `resumeFromCheckpoint()`에서 도착. 클래시 해소는 이동 900ms 뒤, `BoardRenderer`는 880ms 내 런지 스킵. 조준/스킬 arm 0.3배속은 `TickManager.setRate`+`WaveSystem.setTimeScale`.
- **엘리트(라운드 보스) + 최종 보스 + 1차 엔딩**: 런은 5라운드(라운드당 3웨이브). 각 라운드는 엘리트 미니보스로 캡되고(`WaveSystem.ELITE_BY_WAVE`: 웨이브3 피라냐/6 복어/9 청새치/12 고래/13 **상어=최종**), `spawnElite`가 중앙 레인에 스폰(`EnemyToken.isBoss`/`isFinal`/`label`/`attack`). 엘리트는 `board-token.is-boss`로 카드가 더 크고 보라 오라가 숨쉬며, 포획 임계 ≤10%(일반 ≤25%, `captureThreshold`)·확정 카드. 1~4라운드 엘리트는 **처치/포획해도 런 지속(사령 루프 하이라이트)**, **상어(`isFinal`)만 처치(`EncounterResult.isFinal`)/포획 시 `Game.handleVictory`** → 월드 정지 후 700ms 뒤 **1차 엔딩 승리 오버레이**(`runWon` 래치). 일반 스폰은 `ROUND_POOLS`로 라운드별 풀에서 뽑아 얕은 바다→심연으로 하강. 승리/패배 오버레이는 큰 볼드 문구 + 검은 반투명 블러("승리 / 이제 모두 내 친구들이야!…", "패배 / 심해는 무서워!…"). 패배는 디펜스 실패(HP 0). 엘리트 전용 아트는 대기 중(id 기반 자동 연결).
...
- **크리처는 적/아군 두 패시브를 각각 가짐**(before/after). 각각 소수의 와이어드 메카닉에 테마 매핑되고 설명 문구는 크리처별 고유. **아군 패시브**(`passiveId`, 배치 시): **감전 점막**=칸 적 피해 +1(`DefenderHooks.getDamageAmp`→`WaveSystem.damageEnemy`, 시안 스파크)/**폭신 도약**=처치 시 인접 아군 회복(초록 힐)/**단단한 등껍질**=배치 시 +체력(`DefenderSystem.place`의 crab-guard 분기, 앰버 실드) — `PassiveEvent`→`Game.handlePassive`→`BlastManager.passiveBurst`. **적 패시브**(`enemyPassiveId`, `WaveSystem`에서 작동): **ferocious**(맹공, 포식자)=아군/플레이어 타격 시 피해 +1(`resolveClashAt`)/**armor**(방어, 갑각·거구)=받는 피해 -1 최소1(`damageEnemy`)/**regen**(재생, 무리·재생형)=교전 아닐 때 행진하며 `ENEMY_REGEN` 회복(`stepEnemy`). 인스펙터는 적 hover 시 `enemyPassive`, 아군 hover 시 `passive`를 표시하고 도감은 before/after 각 면에 해당 패시브를 보여준다. 크리처 19종 시드(해파리·바다토끼·꽃게 + 신규 16종), 아트 3종만 실제 원화·나머지 대기(폴백 워터마크).

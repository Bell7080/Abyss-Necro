...
- `AbilitySystem`: 코스트 풀(최대10, 1.2초당 +1). 기본 공격(코스트1)은 조준 단계 없이 보드 칸을 클릭하는 즉시 발동하고, 스킬(코스트10, 생존 적 전체)은 스킬 아이콘을 직접 클릭해야만 발동한다. 둘 다 `CurseMortar`로 본진 카드에서 곡선 발사되어 착탄 시에만 실제 피해가 들어간다.

...
- 묘지(별): 소환된 크리처 아군 사망 시 금색 별이 좌측 `GraveyardPanel`로 날아가 보관(`GraveyardSystem`, `DefenderSystem.onAllyDeath`→`Game.handleAllyDeath`→`BlastManager.starFly`). hover 시 "○○의 별". 소환수는 별 없음. 라운드 종료 이탈(`Game.resumeRun`) 시 같은 크리처 별 3개→온전한 카드 1장으로 회수해 손패로(`combineTriples`).

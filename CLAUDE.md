...
- `AbilitySystem`: 코스트 풀(최대10, 1.2초당 +1). 기본 공격(코스트2)은 조준 단계 없이 보드 칸을 클릭하는 즉시 발동하고, 스킬(코스트10)은 스킬 아이콘을 직접 클릭해야만 발동한다. 둘 다 즉발 피해 대신 `DefenderSystem.summon()`으로 보스 칸 옆(그리드 0열)에 임시 소환수를 등장시킨다 — 기본은 "멸치 소환수"(hp2/공격1/이동 12칸), 스킬은 "정예 소환수"(hp8/공격4/이동 30칸)이며 `stepSummons()`가 매 틱 적과 같은 방식으로 이동시키다 이동 횟수가 다하면 소멸시킨다. 이전 즉발 투사체(`CurseMortar`)·데미지 숫자(`FloatingDamage`)·`WaveSystem.applyDamage`는 현재 미사용이지만 추후 재사용을 위해 코드는 보존돼 있다.
- `DefenderSystem`: 손패 카드를 선택해 칸(적이 있어도 무관, 칸당 최대 3기)에 배치하면 아군 디펜더가 된다. 전투는 적과 아군이 실제로 같은 칸에 있을 때만 시작되어 매 틱 서로 피해를 교환한다.

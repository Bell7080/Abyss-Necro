...
- `WaveSystem`: 적은 그리드 우측 열(본진 반대편)에서 스폰돼 매 틱 본진 방향으로 전진/배회한다. 처치(클릭 공격 또는 본진 도달)는 50% 확률 카드 획득 + 3웨이브마다 유물 1개 규칙으로 귀결된다.
- `AbilitySystem`: 코스트 풀(최대10, 1.2초당 +1). 기본 공격(코스트1, 단일 칸 조준 후 클릭)과 스킬(코스트10, 생존 적 전체)은 `CurseMortar`로 본진 카드에서 곡선 발사되어 착탄 시에만 실제 피해가 들어간다.
- `DefenderSystem`: 손패 카드를 선택해 빈 칸에 배치하면 아군 디펜더(hp6)가 된다. 적이 그 칸으로 이동을 시도하면 서로 피해를 교환하며 진행을 막는다.

...
- `IntroOverlay`: 게임 진입 시 보드가 아닌 별도 타이틀 스크린(배경 아트 `bg_002`)을 먼저 보여준다. 완전한 검은 화면(`.intro-black-veil`)이 2.8초에 걸쳐 걷히며 타이틀 아트 + 캐스틱 물결(`.abyss-caustic` 재사용) + 강한 비네틱이 드러나고, 다 드러난 뒤(`onRevealed` 콜백)에야 OST 재생이 시작되며(6초 페이드인) 하단에 "화면을 눌러 심해의 친구들을 사귀러 떠나기!"가 점멸한다. 화면 아무 곳이나 클릭하면 장면 전환처럼 화면이 다시 검게 컷되며(`onDismiss` 콜백에서 `AudioManager.duckTitleOst()`가 OST를 "투-후-후-훙" 스타터 컷으로 끊는다) 그 암전 뒤에서 다시 페이드인되어 아직 정지된 채인 보드가 드러나고, 중앙에 "시작하기" 버튼이 뜬다 — **이 버튼을 눌러야 비로소** `Game.startRun()`이 호출되어 `WaveSystem.start()`/`AbilitySystem.start()`가 웨이브 스폰·이동·게이지 트리클을 시작한다 — 그 전까지 보드는 비어 있고 웨이브 HUD도 정지해 있다.

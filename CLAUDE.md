...
- 카드 3합성(1→2→3성)·보스 포획·도감 비포/애프터·5라운드 엘리트/최종 보스(상어)·승리 조건은 구현됨. 크리처 19종 시드 + 라운드별 스폰 풀도 배선됨. **아직 미구현**: 신규 16종 크리처 아트(id 기반 자동 연결 대기 — webp만 넣으면 점등), 진화체 고유 아트(현재 2·3성은 원본 아트 + 금색 오라), 40/40/20 풀 로스터로의 확장, 크리처별 고유 패시브 메카닉 다양화(현재 3종 와이어드 재사용). 밸런스 수치(등급 스탯/게이지/적 스케일/엘리트 스탯)는 시작값이라 플레이로 조정 대상.
